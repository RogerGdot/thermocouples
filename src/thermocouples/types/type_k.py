...
def _type_k_seebeck_expo_function(temp_c: float) -> float:
    """
    Exponential correction function for Type K Seebeck coefficient calculation.

    This implements the additional term in the Seebeck coefficient formula:
    (-0.0280704)*exp(-0.000118343*((T - 126.9686)*(T - 126.9686)))*(T-126.969)

    Args:
        temp_c: Temperature in degrees Celsius

    Returns:
        Exponential correction term for Seebeck coefficient in µV/K
    """
    # $ Source: Derivative of _type_k_voltage_expo_function
    # Derivative of the function: a0 * exp(a1 * (T - a2)^2)
    # Product rule: f'(T) = a0 * exp(a1 * (T - a2)^2) * 2 * a1 * (T - a2)
    # The differentiated coefficients:
    if temp_c > 0:
        da0 = -0.0280704  # Derivative of a0
        da1 = -0.000118343  # Derivative of a1
        da2 = 126.9686  # Derivative of a2
        result = da0 * math.exp(da1 * (temp_c - da2) ** 2) * (temp_c - 126.969)
        return result
...
def _type_k_dsdt_expo_function(temp_c: float) -> float:
    """
    Exponential correction function for Type K dSeebeck/dT calculation.

    This implements the additional exponential term in the dSeebeck/dT formula:
    exp(-0.000118343*((T - 126.9686)*(T - 126.9686))) * (6.64387E-6 * T^2 - 0.00168713 * T + 0.0790359)

    Args:
        temp_c: Temperature in degrees Celsius

    Returns:
        Exponential correction term for dSeebeck/dT in µV/K²
    """
    # $ Source: Derivative of _type_k_seebeck_expo_function
    if temp_c > 0:
